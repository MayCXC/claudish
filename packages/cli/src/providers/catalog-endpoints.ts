 ...
 *
 * An override replaces the host and the `catalog=slim` view. The parameters
 * every page needs (status, route variants, limit, cursor, generation) are set
 * by `buildCatalogPageUrl` on whatever URL is in effect, so an override cannot
 * drop them.
 */
const DEFAULT_CATALOG_URL = `${DEFAULT_HOST}/queryModels?status=all&catalog=slim&includeRouteVariants=true&limit=1000`;

/** The catalog endpoint in effect: `CLAUDISH_CATALOG_URL`, else the published one. */
